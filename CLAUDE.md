...
- Next up: **applying** movement inside `resolve()` — mutate state and emit `UNIT_MOVED` plus new events for standoffs/blocked advances, implementing spec §9. The rules are settled; the event type shapes still need defining and approving first.
- Rules decided this session (all now written into **spec §9** — that is the source of truth, not this summary): occupied tiles block passage *and* landing, destroyed units block nothing; moving to your own hex is illegal, not a no-op; two units racing for the same empty hex both bounce (standoff); a move blocked by a hidden enemy fails entirely with no partial advance; all moves resolve against round-start state, so no chaining/following and swaps are illegal.
- Open questions: (1) HP for launcher/radar/interceptor isn't in the spec (only the Leader's "2 penetrating hits") — assuming 1, unconfirmed. (2) `MISSILE_DEFS` doesn't exist yet — needed for the launch/interception sessions. (3) How much a failed/standoff move reveals in the event log — spec §9 says surface "advance halted" without naming the blocker, but the exact `GameEvent` shape is undecided.
