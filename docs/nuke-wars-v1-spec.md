...
- **MARCH** a launcher — a **forced march**: the same ground move on `RULES.forcedMarchMovement` (6) instead of the launcher's movement 3, paid for by **going loud**. The hex the launcher *leaves* is detected automatically and unsuppressably, exactly as a launch origin is (§9, §11)
...
**There are two loud actions: launching and forced-marching.** Everything else a player does is silent. This is the axis the whole detection system runs on (§11) — you may be found by the enemy's drone, or you may announce yourself, and announcing yourself is always a choice made in exchange for something.
...
| `MARCH_DETECTED` (owner, origin) | **Both** — the second loud action, on the same terms (§9, §11). Carries the origin and **never the destination**: publishing where the launcher arrived would hand over its current position, which is the whole thing the reveal is meant to leave unsaid. Marks a one-round launcher contact on the *other* player's map. Emitted in ascending origin-hex order, never units order (§9). Carries `owner` even though `LAUNCH_DETECTED` does not — not a leak, since both recipients can already derive it from knowing their own orders, and it saves the client inferring it |
...
**What the visibility filter hands back** (`src/sim/visibility.ts`, build-order step 8). `filterForPlayer(state, playerId)` returns a `VisibleGameState` and `filterEventsForPlayer(events, playerId)` returns `VisibleEvent[]`. Wherever a redaction can be made *structural* it is: the output types are built so the hidden thing cannot be expressed, rather than merely omitted by careful code.

- **Enemy units are absent, not masked.** The filtered `units` array holds the viewer's own units only — including their own destroyed ones, since your losses are your own knowledge. Everything the viewer knows about the enemy lives in `intel`, keyed by hex. `VisibleGameState` therefore has no field capable of carrying an enemy `UnitId` at all, which makes §11's "you learn a place, not an identity" a property of the type rather than a rule someone has to keep.
- **`intel` narrows to the viewer's own picture**, and its static reveals carry `MaskedStaticKind`, in which `'decoy'` is not expressible (§12). Launcher contacts pass through untouched: a contact is a hex plus its source, with no identity to mask.
- **`droneRespawnIn` narrows to the viewer's own counter.** `DRONE_RESPAWNED` is owner-only precisely so the enemy cannot time your recon coming back online; leaving their counter in the state would hand them the same fact directly and make filtering the event pointless.
- **`map`, `round`, `phase`, `deadHandFor` and `outcome` pass through unredacted.** Terrain is public (§11), and the other four are described by events both players receive — hiding them would only desynchronise a client from a log it already holds.
- **It is a projection, not a deep clone.** Arrays are rebuilt so a caller cannot splice into the real state; the objects inside are shared wherever nothing is masked, because the presentation layer never mutates game state and copying every unit per frame is waste.
- **Two engine invariants throw rather than degrade gracefully**: an `ASSET_SPOTTED` naming a drone (no detector may reveal one, §2/§11), and an event kind with no assigned audience. Both mean the engine broke its own rules, and a filter that quietly relabelled or dropped them would hide the bug behind a screen that still works.

**The filter decides nothing.** `resolve()` already does every piece of intel bookkeeping — filing contacts, rebuilding the one-round pile from scratch each resolution, and forgetting a hex when a unit is publicly destroyed (§11) — so `state.intel[playerId]` is already exactly what that player knows, and this layer only projects it. If a rule about *whether* something is visible ever needs writing, it belongs in `resolve()`.
...
| Forced march | 6 hexes/round (`RULES.forcedMarchMovement`) instead of 3, in exchange for a public `MARCH_DETECTED` on the hex left behind (§9, §11) |
...
| Launcher sighting (recon, detected launch, or detected march) | Visible for **1 round** — the next order phase only (§11) |
...
**Why the forced march is 6 and not 4 or 5** (added 2026-08-13, untested like every other number here). It is meaningless in isolation and only matters against the 8-cost approach from a launcher spawn to a firing position: at 3 that approach takes 3 rounds, at 6 it takes 2. Setting it equal to `missileRange` makes the geometry learnable from a single fact — **the disc the enemy must blind-fire into to catch a marcher is exactly the disc one launcher already covers** — so a player who understands missiles already understands what a march contact is worth. Raising it further starts to outrun counter-battery entirely and turns the reveal into a formality; lowering it toward 4 makes the reveal cost more than the hexes are worth and the order simply stops being played. Both failure modes are visible in `npm run soak` as the share of matches in which a MARCH is ever ordered, so this is a number with a measurement attached rather than a guess — **but the measurement needs a CPU that marches, which is a later session.**
...
### The forced march (MARCH) — added 2026-08-13

**A forced march is a MOVE on a bigger budget that announces the hex it left.** `RULES.forcedMarchMovement` (6) replaces the launcher's movement 3; everything else about the order is identical, and that identity is the rule. Same flood fill, so a march is stopped by mountains and blocked by living ground units exactly as a walk is — it is emphatically **not** a `distance <= 6` check, and turning it into one would walk launchers through ridges. Same start-of-phase snapshot, same standoff adjudication, same `MOVE_FAILED` on the two contact reasons. MARCH is launcher-only for the same reason MOVE is, and rejects the drone with the same `AIR_UNIT`.

Implementation follows: `validateMove` and `validateMarch` are two names for one function on two budgets (`groundBudget` in `src/sim/movement.ts`), because two separate validators would be two places for the mountain rule to drift apart.

**The price is a public `MARCH_DETECTED` on the origin hex**, which files a one-round launcher contact on the enemy's map (§11) — the mobile rule, same as every other launcher sighting.

**The reveal fires on the attempt, not on arrival.** A march blocked by an unseen enemy, or one that loses a standoff, is heard anyway: the launcher revved up whether or not it got anywhere. The precise boundary is "any march that had an effect on the round" — it moved, or it earned a `MOVE_FAILED`. A march dropped for a reason the ordering player could already see (a mountain destination, a dead unit, an enemy's unit id) is a client bug rather than gameplay, is dropped in silence exactly as the equivalent MOVE is, and is heard by nobody. This mirrors the `MOVE_FAILED` split above and is deliberate: the two rules have the same boundary because they answer the same question — did hidden information cause this?

**A march shorter than the walking budget is legal.** Paying a public reveal for three hexes looks like a mistake and is a real tactic: the enemy is told a hex you have *already left*, and may spend a counter-battery volley on empty ground. Rejecting it would delete the feint for no gain.

**What the enemy actually buys is deliberately weaker than a launch contact.** A launcher that fires cannot also move, so a launch origin is a live target and counter-battery lands in phase 3 before it can run (§3, §11). A march origin is the opposite — **stale by construction**, since the launcher has by definition just left. The enemy gets a bearing and an axis of advance, not a kill. That is the right exchange rate for a movement buff, and it is what makes marching *out* of a hex usable as a bluff.

**`MARCH_DETECTED` is emitted in ascending origin-hex order, not `GameState.units` order** — see the emission-order rule at the end of this section, which the march forced to grow a second half.
...
**But the public event in this phase is ordered by hex, not by units** (added 2026-08-13 with the forced march). `UNIT_MOVED` and `MOVE_FAILED` are owner-only, so units order leaks an ordering to nobody. `MARCH_DETECTED` is **public**, and units order would publish the relative unit ids of the marching launchers — two marches in a round would tell the enemy which of two launcher ids sorts first, and a few such rounds reconstruct the ordering of all three. That is exactly the cross-round trackable identity §11 keys every scrap of intel by hex to withhold, and it is the same leak that moved the missile tiebreak off unit ids in §10. `MARCH_DETECTED` is therefore emitted in **ascending origin hex** (`compareHex`), as one group, before the owner-only movement events. Origin hexes are public the instant the event fires, so ordering by them tells the enemy nothing they were not already handed.

The general rule this makes explicit, worth applying to any future event: **an owner-only event may be ordered by units; a public one must be ordered by something already public.**
...
2. **An enemy asset appears on your map only when you detect it.** There are exactly two detectors: your **recon drone's swath** and **automatic emission detection**. Nothing else on the board reveals an enemy asset — no adjacency, no line of sight, no "you can see 2 hexes around your units." Emission detection fires on the game's two **loud actions** (§3): **launching**, which publishes the firing hex, and **forced-marching**, which publishes the hex left behind. Both are automatic, universal and unsuppressable; neither needs equipment; nothing can be done to prevent one except to not take the action.
...
| **Emission detection — launch** | Phase 2 | The origin hex of every enemy missile fired this round (`LAUNCH_DETECTED`, §6). Automatic and unavoidable — launches are loud; no equipment is needed to detect one and nothing can suppress it | 1 round — a launcher can relocate next round, so the marker expires with it |
| **Emission detection — forced march** | Phase 5 | The hex every enemy launcher **left** on a MARCH order this round (`MARCH_DETECTED`, §6, §9). Same terms as a launch, and never the destination | 1 round, same rule |

Both emission contacts are *launcher* sightings, so they follow the mobile rule. The `LAUNCH_DETECTED` and `MARCH_DETECTED` events stay in your log permanently (below); the map markers do not.

**The two emission contacts are worth very different amounts, and the source field says which is which.** A launch origin cannot be stale — a launcher that fired could not also move — so it is a live target for exactly one round. A march origin is **stale by construction**: the launcher announced that it was leaving, so it is somewhere within `RULES.forcedMarchMovement` of that hex and certainly not on it. One is a target; the other is a bearing. Firing at a march contact is not merely a bet, as a recon contact is, but a shot at a hex you have been *told* is empty — which is what makes marching out of a hex a usable feint (§9).

**A launcher caught by two detectors in one round is still one contact, keyed by hex, and the first source to file it wins** (see below). A launcher that was photographed by recon in phase 1 and then force-marched out of the same hex in phase 5 is filed `RECON`, not `MARCH`. Accepted deliberately rather than special-cased: both labels are true statements about that hex, the difference is UI flavour, and the alternative is a precedence table that has to be kept in step with the phase order.
...
- **A march origin is always stale, and you know it.** The contact is filed in phase 5 by a launcher that has just driven off it, so it is the one sighting that is guaranteed wrong about where the enemy *is*. What it is right about is where they were and roughly where they can be: somewhere within `RULES.forcedMarchMovement` of that hex, which is the same disc a single launcher already covers with a missile. Read it as a heading, not a target.
...
**Amendment, 2026-08-11 — §6 gains the visibility filter's output contract (build-order step 8).** §6 named `filterForPlayer` and `filterEventsForPlayer` and specified, in the event table, exactly who may see what — but never said what the *filtered state* actually contains, which turned out to be the half with the leaks in it. No rule, number or audience changed; six rulings were added (see "What the visibility filter hands back" above). The load-bearing one is that **enemy units are absent from the filtered state rather than masked within it**: the whole enemy picture is `intel`, keyed by hex, so `VisibleGameState` has no field that could carry an enemy `UnitId` and §11's withheld identity becomes a property of the type. The others follow the same principle of making a redaction structural where possible — `MaskedStaticKind` in the reveals, a bare `droneRespawnIn` for the viewer alone — with `map`/`round`/`phase`/`deadHandFor`/`outcome` explicitly passing through, since events both players already receive describe them. Two engine invariants now throw rather than degrade: an `ASSET_SPOTTED` naming a drone, and an unrouted event kind. Confirmed by implementation and left unchanged: the filter needs no `GameState` to route events, which is what lets a whole match's replay be re-filtered for either player without rebuilding each round's state.

**Amendment, 2026-08-13 — the forced march (MARCH), and §11 rule 2 generalised from "launch detection" to "emission detection".** A new launcher order: move on `RULES.forcedMarchMovement` (6) instead of movement 3, and the hex you *leave* becomes a public `MARCH_DETECTED` filing a one-round launcher contact on the enemy's map (§3, §9, §11). **This is the only rule in V1 that adds a way for an enemy asset to reach your map**, so it was written as a generalisation rather than an exception: rule 2 still says "exactly two detectors", but the second is now emission detection, which fires on the game's two **loud actions** — launching and forced-marching. The detector count, rule 3's mobile/static lifetimes, and rule 4's bunker/decoy indistinguishability are all untouched, and §12 is untouched entirely because the march concerns launchers only.

Five rulings came with it. (1) **The reveal fires on the attempt, not on arrival** — a blocked or standoff march is heard anyway; the boundary is "any march that had an effect on the round", which is deliberately the same boundary `MOVE_FAILED` already uses, because both answer "did hidden information cause this?" (§9). (2) **`MARCH_DETECTED` carries the origin and never the destination** — publishing the arrival hex would hand over the launcher's current position and delete the feature. (3) **A short march is legal** — paying the reveal for three hexes is the feint, not a mistake, since the enemy is told a hex you have already left. (4) **`MARCH_DETECTED` is emitted in ascending origin-hex order, never `GameState.units` order.** It is the first *public* event in phase 5, and units order would have published the relative unit ids of the marching launchers — the same cross-round trackable-identity leak that moved the §10 missile tiebreak off unit ids. This generalised §9's emission rule: **an owner-only event may be ordered by units; a public one must be ordered by something already public.** (5) **A launcher photographed and then marched out of the same hex stays one contact, filed `RECON`** — first source wins, as intel keyed by hex already required; the difference is UI flavour and a precedence table would be one more thing to keep in step with the phase order.

The design property that makes the price fair is worth stating plainly, because it is not obvious from the numbers: **a launch contact is a target and a march contact is a bearing.** A launcher that fires cannot also move, so its origin is live and counter-battery lands before it can run (§3). A launcher that marches has by definition left. The enemy buys your axis of advance, not your position — which is the right exchange rate for a movement buff, and is what makes marching out of a hex usable as a bluff.

`RULES.forcedMarchMovement` is a first draft like every other §7 number, set equal to `missileRange` so the counter-battery disc is the one players already know. Implemented and tested in the sim (`src/sim/march.test.ts`, 24 tests, five invariants mutation-checked); **not yet reachable from the UI or the CPU**, which is a later session — `modesFor` in `src/state/orders.ts` is the single gate, and until 'MARCH' is added there the rule is inert. That inertness was verified rather than assumed: `npm run soak` printed byte-identical output before and after the change.
