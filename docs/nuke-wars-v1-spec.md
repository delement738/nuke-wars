...
|---|---|---|---|
| **Recon drone swath** | Phase 1 | Every enemy asset in the swath — launchers, interceptor bases, and bunker/decoy sites (**but see the geometry note below: at the current radii, a base can never actually appear in a swath**) | Launchers: 1 round. Static assets: permanent |
...
### OPEN QUESTION — recon cannot currently photograph an interceptor base

Found while implementing build-order step 5, and **not yet decided**.
...
Two consequences, both live in the current build:

- The detector table above overstates what recon does. Bases are findable only by *inference* from the public 7-candidate clues, `MISSILE_INTERCEPTED` and `DRONE_DOWNED` — and a drone dying is itself the strongest such clue.
- §12's design note *"finding interceptor bases narrows the bunker hunt… a genuine second use for drone intel"* is dead as written. The ≥3 exclusion still holds, so the inference is still available; what is gone is recon's ability to confirm it.

The options, for a playtest decision:

1. **Leave it at 1.** Accept that bases are inferred, never photographed, and reword the detector table and the §12 note to match. Recon stays purely a launcher/bunker hunter, and the drone dying *is* the base-detection mechanic (you pay a drone for a 7-hex answer).
2. **Raise `reconSwathRadius` to 2.** A one-number change in `defs.ts`: the drone can then fly past at distance 2, outside the bubble, and photograph the base. This restores §12's second use for drone intel — but it also widens *every* reveal from a 3-wide corridor to a 5-wide one, which is a large across-the-board buff to recon and shortens the bunker hunt considerably.
3. **Cut coverage to a base's own hex for drones only** (keep radius 1 vs missiles). Preserves the swath width, but splits one rule into two and is a §12-style asymmetry waiting to be exploited.

Nothing in the engine assumes an answer — the radius is a data-table number, and `defs.ts` carries the warning at both constants.
...
- *Finding interceptor bases narrows the bunker hunt.* Because bases must sit ≥3 from both sites, an attacker who spots both bases can rule out every hex within 2 of either. Recon that finds defenses is therefore also recon that finds the leader — a genuine second use for drone intel. **⚠ Currently only reachable by inference:** at the shipped radii a drone dies one hex before it can photograph a base, so the attacker gets a 7-candidate clue from `DRONE_DOWNED` rather than a confirmed location. See "OPEN QUESTION — recon cannot currently photograph an interceptor base" in §11.
